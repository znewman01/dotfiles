...
(after! deft
  (setq deft-directory "~/Sync/notes/roam"
        deft-recursive t))
...
#+begin_example
#+begin_src bash
hello:shebang (zjn/with-pkgs "hello") :results verbatim
#+end_src
...
#+RESULTS:
| #s(mu4e-context Fastmail nil nil (lambda (msg) (when msg (string-prefix-p /fastmail (mu4e-message-field msg :maildir)))) ((mu4e-trash-folder . /fastmail/Trash) (mu4e-sent-folder . /fastmail/Sent) (mu4e-drafts-folder . /fastmail/Drafts) (mu4e-refile-folder . /fastmail/Archive) (user-mail-address . z@znewman.net) (user-full-name . Zachary Newman) (smtpmail-local-domain . znewman.net) (smtpmail-smtp-server . smtp.fastmail.com) (smtpmail-stream-type . ssl) (smtpmail-smtp-service . 465))) | #s(mu4e-context MIT nil nil (lambda (msg) (when msg (string-prefix-p /mit (mu4e-message-field msg :maildir)))) ((mu4e-trash-folder . /mit/Deleted) (mu4e-sent-folder . /mit/Sent) (mu4e-drafts-folder . /mit/Drafts) (mu4e-refile-folder . /mit/Archive) (user-mail-address . zjn@mit.edu) (user-full-name . Zachary Newman) (smtpmail-local-domain . mit.edu) (smtpmail-smtp-server . outgoing.mit.edu) (smtpmail-stream-type . ssl) (smtpmail-smtp-service . 465))) | #s(mu4e-context CSAIL nil nil (lambda (msg) (when msg (string-prefix-p /csail (mu4e-message-field msg :maildir)))) ((mu4e-trash-folder . /csail/Trash) (mu4e-sent-folder . /csail/Sent) (mu4e-drafts-folder . /csail/Drafts) (mu4e-refile-folder . /csail/Archive) (user-mail-address . zjn@csail.mit.edu) (user-full-name . Zachary Newman) (smtpmail-local-domain . csail.mit.edu) (smtpmail-smtp-server . outgoing.csail.mit.edu) (smtpmail-stream-type . starttls) (smtpmail-smtp-service . 587))) | #s(mu4e-context Gmail nil nil (lambda (msg) (when msg (string-prefix-p /gmail (mu4e-message-field msg :maildir)))) ((mu4e-trash-folder . /gmail/[Gmail]/Trash) (mu4e-sent-folder . /gmail/[Gmail]/SentMail) (mu4e-drafts-folder . /gmail/[Gmail]/Drafts) (mu4e-refile-folder . /gmail/[Gmail]/AllMail) (user-mail-address . znewman01@gmail.com) (user-full-name . Zachary Newman) (smtpmail-local-domain . gmail.com) (smtpmail-smtp-server . smtp.gmail.com) (smtpmail-stream-type . starttls) (smtpmail-smtp-service . 587))) |
...
(let* ((trash-folders (zjn--get-mu4e-vars 'mu4e-trash-folder))
       ...
       (skip-trash-and-sent
         (lambda (query) (funcall query-skipping query (-concat trash-folders sent-folders '("/mit/Junk" "/gmail/[Gmail]/Spam"))))))
    (setq mu4e-bookmarks
...
#+begin_src emacs-lisp
(require 'mu4e-mark)
; Don't ask for confirmation when executing (TODO: does this work?)
...
#+begin_src emacs-lisp
(setq mu4e-alert-interesting-mail-query "flag:unread AND NOT flag:trashed AND NOT maildir:/mit/Deleted")
#+end_src
** Outbox
*** Composing
[[org]] everywhere!
